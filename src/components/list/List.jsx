...
const List = props => {
    const {title, type, tasks, addNewTask, blTasks, setTasks, allTasks, ipTasks, rdTasks} = props
        ...
        <div className={css.list}>
            ...
            {type === LIST_TYPES.READY &&
             ...
             {type === LIST_TYPES.IN_PROGRESS &&
                  (<button className={css.addButton} onClick={handleClick} disabled= {rdTasks.length === 0 } style={{ display: !isFormVisible ? "block" : "none" }}>+Add card</button>)
            }
            {type === LIST_TYPES.FINISHED &&
                  (<button className={css.addButton} onClick={handleClick} disabled= {ipTasks.length === 0 } style={{ display: !isFormVisible ? "block" : "none" }}>+Add card</button>)
            }
